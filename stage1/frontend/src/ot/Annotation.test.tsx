...
const t = makeT("en");
const ITEM: QueueRow = { submission_id: "s1", display_code: "CH-AAAAAA", scenario: "name", size_bytes: 1, state: "pending", created_at: "" };
...
function setup() {
  ...
  const go = vi.fn();
  render(<Annotation t={t} go={go} item={ITEM} annotate={annotate} toast={() => {}} />);
  return { annotate, go };
}
